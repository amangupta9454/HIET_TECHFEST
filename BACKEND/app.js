...
//         const events = await DataRegisterModel.find({ $or: [{ email: user.email }, { 'teamMembers.email': user.email }] });
...
//     { name: "clg_id" },
//     ...Array.from({ length: 10 }, (_, i) => ({ name: `teamMembers[${i}][clg_id]` }))
...
//         uploadPromises.push(cloudinary.uploader.upload(
...
//             }
//             uploadPromises.push(cloudinary.uploader.upload(
//                 req.files[`teamMembers[${index}][clg_id]`][0].path,
//                 {
...
//         const collegeIdUrl = uploadResults[0].secure_url;
//         const teamMemberUrls = uploadResults.slice(1);
...
//         });
//         for (let index = 0; index < parsedTeamMembers.length; index++) {
//             fs.unlink(req.files[`teamMembers[${index}][clg_id]`][0].path, (err) => {
//                 if (err) console.log("Error deleting team member college id file:", err);
//             });
//         }

//         const uploadedTeamMembers = parsedTeamMembers.map((member, index) => ({
//             ...member,
//             clg_id: teamMemberUrls[index].secure_url,
