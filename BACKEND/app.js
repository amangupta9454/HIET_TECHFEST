...
`;
  ...
  try {
    console.log(`📧 Attempting to send OTP to ${email}...`); // Log start
    ...
    console.error("❌ Failed to send OTP email:", {
      message: err.message,
      code: err.code,
      response: err.response, // SMTP response code (e.g., 535 for auth fail)
      stack: err.stack
    });
    return false;
  }
...
// ... (rest of email templates unchanged - generateLeaderEmailTemplate, etc.)
...
app.post('/api/user/register', registerLimiter, validateUserRegistration, async (req, res) => {
    ...
    try {
        ...
        await user.save();
        console.log(`👤 User saved to DB with OTP: ${otp} (expires: ${user.otpExpires})`); // Log DB success

        // Temporarily sync: Await email send to catch errors (switch to .catch() for production speed)
        const emailSent = await sendOTPEmail(email, otp);
        if (!emailSent) {
            return res.status(500).send({ message: 'Failed to send OTP email. Please try again or contact support.' });
        }
...
// ... (rest of routes unchanged - verify-otp, login, dashboard, register/check, register)

app.post('/api/test-email', async (req, res) => {
    // New test endpoint: POST { "email": "test@example.com" } to verify SMTP
    const { email } = req.body;
    if (!email) {
        return res.status(400).send({ message: 'Email required for test' });
    }

    try {
        const testOtp = generateOTP();
        const sent = await sendOTPEmail(email, testOtp);
        if (sent) {
            res.status(200).send({ message: 'Test email sent successfully!', otp: testOtp }); // Include OTP for manual verification
        } else {
            res.status(500).send({ message: 'Test email failed. Check server logs.' });
        }
    } catch (err) {
